document.addEventListener("DOMContentLoaded", () => {
  ...
  function draw() {
    ...
    if (newWaypoints.length > 0) {
      ...
      for (let i = 1; i < newWaypoints.length; i++) {
        ctx.lineTo(
          mapToCanvasCoords(newWaypoints[i]).x,
          mapToCanvasCoords(newWaypoints[i]).y,
        );
      }
  ...
  function mapToCanvasCoords(wp) {
    ...
    const y =
      mapMetadata.height -
      (wp.y - mapMetadata.origin[1]) / mapMetadata.resolution;
    return { x, y };
  }
  ...
  function canvasToMapCoords(p) {
    ...
    const y =
      (mapMetadata.height - p.y) * mapMetadata.resolution +
  ...
  function showEditPopup(waypointData, index) {
    ...
    modalTitle.textContent =
      index === null ? "Add New Waypoint" : `Edit Waypoint ${index + 1}`;
  ...
  waypointForm.addEventListener("submit", (e) => {
    ...
    const updatedWaypoint = {
      ...
      curvature:
        index !== null && newWaypoints[index]
          ? newWaypoints[index].curvature
          : 0,
    };
    if (index === null) {
      newWaypoints.push(updatedWaypoint);
    } else {
  ...
  function getWaypointAt(p) {
    for (let i = newWaypoints.length - 1; i >= 0; i--) {
      ...
      const dist = Math.sqrt(
        Math.pow(wpCanvas.x - p.x, 2) + Math.pow(wpCanvas.y - p.y, 2),
      );
      if (dist < 10) {
        return i;
      }
  ...
  editorContainer.addEventListener("dblclick", (e) => {
    ...
    if (waypointIndex !== -1) {
      ...
      const mapCoords = canvasToMapCoords(mousePos);
      showEditPopup(
        { x: mapCoords.x, y: mapCoords.y, theta: 0, curvature: 0 },
        null,
      );
    }
  });
  ...
  document.getElementById("export-csv").addEventListener("click", async () => {
    ...
    const blob = await exportData("/api/export_csv", {
